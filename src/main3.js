...
const CFG = {
  ...
  bendRadius: 1.0,
  // 中央スケール設定
  scaleMin: 0.7,        // 遠いカードの最小スケール
  scaleMax: 1.2,        // 中央カードの最大スケール
  scaleFadeRange: 3.0,  // この距離以内でスケール補間
};
...
for (let copy = 0; copy < COPIES; copy++) {
  for (let i = 0; i < totalCards; i++) {
    ...
    mesh.userData = {
      ...
      project: proj,
      currentScale: CFG.scaleMin, // lerp 用の現在値
    };
...
function animate() {
  ...
  cards.forEach(card => {
    ...
    // 中央スケール — 中心(y=0)に近いほど大きく
    const t = Math.max(0, 1 - dist / CFG.scaleFadeRange); // 0〜1
    const targetScale = CFG.scaleMin + (CFG.scaleMax - CFG.scaleMin) * t * t; // easeIn
    card.userData.currentScale = lerp(card.userData.currentScale, targetScale, 0.1);
