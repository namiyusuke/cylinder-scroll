...
// cardStep, loopHeight はジオメトリ生成後に確定
let cardStep, leftLoopHeight, rightLoopHeight;
...
// ============================================================
// Plane card geometry (2列用に幅を縮小)
// ============================================================
const fullWidth = 2 * CFG.radius * Math.sin(CFG.arcAngle / 2);
const cardWidth = fullWidth;
const cardHeight = cardWidth / (1440 / 900); // 1440:900 比率を維持
cardStep = cardHeight + CFG.cardGap;
leftLoopHeight = leftProjects.length * cardStep;
rightLoopHeight = rightProjects.length * cardStep;
...
const COPIES = 3;
const columnOffset = (cardWidth + CFG.columnGap) / 2;

// 左列
for (let copy = 0; copy < COPIES; copy++) {
  for (let i = 0; i < leftProjects.length; i++) {
    const proj = leftProjects[i];
    ...
    const baseY = i * cardStep + copy * leftLoopHeight;
    mesh.position.x = -columnOffset;
...
// 右列
for (let copy = 0; copy < COPIES; copy++) {
  for (let i = 0; i < rightProjects.length; i++) {
    const proj = rightProjects[i];
    const tex = loadCardTexture(proj.image);
    const mat = new THREE.ShaderMaterial({
      uniforms: {
        map:         { value: tex },
    ...
    const mesh = new THREE.Mesh(sharedGeo, mat);
    const baseY = i * cardStep + copy * rightLoopHeight;
    mesh.position.x = columnOffset;
    mesh.position.y = baseY;
...
gui.add(brightnessCtrl, 'brightness', 0, 3, 0.05).name('brightness').onChange(v => {
  allCards.forEach(c => { c.material.uniforms.uBrightness.value = v; });
});
...
function animate() {
  ...
  // Update left column
  const leftLoopTotal = leftLoopHeight * COPIES;
  const leftHalfLoop = leftLoopHeight * 1.5;
  leftCards.forEach(card => {
    card.material.uniforms.uTime.value = elapsedTime;
    let y = card.userData.baseY - scrollCurrent * card.userData.parallaxFactor;
    y = mod(y + leftHalfLoop, leftLoopTotal) - leftHalfLoop;
  ...
  // Update right column (同じ方向にスクロール)
  const rightLoopTotal = rightLoopHeight * COPIES;
  const rightHalfLoop = rightLoopHeight * 1.5;
  rightCards.forEach(card => {
    card.material.uniforms.uTime.value = elapsedTime;
    let y = card.userData.baseY - scrollCurrent * card.userData.parallaxFactor;
    y = mod(y + rightHalfLoop, rightLoopTotal) - rightHalfLoop;
  ...
  const hits = raycaster.intersectObjects(allCards);
